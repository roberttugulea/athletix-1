...
- 7.4 ✅ Report. `/report` (permesso `reports.read`): tessere KPI (atleti attivi, gruppi, quote mese per stato, da incassare, incassato nel periodo = pagamenti − rimborsi, presenze ultimi 30g %, certificati/tesseramenti in scadenza) con filtro periodo `?from&to`. Export CSV via route handler `(app)/report/export/route.ts` (auth + `reports.read` manuali, no layout) → dataset `payments` / `fees` / `athletes`; `src/lib/csv.ts` `toCsv` (separatore `;`, BOM UTF-8, quoting RFC 4180). Verificato a runtime: KPI corretti (incassato 19,33 € = 29,33 − 10,00), 3 CSV scaricati con header e dati giusti.
- 7.6 ✅ Ricerca globale. Barra `<form action="/cerca">` nella topbar dell'`AppShell`; `/cerca?q=` cerca (ilike, ≥2 caratteri, filtrata per permesso) su atleti, coach, gruppi, gare e mostra i risultati raggruppati con link. Verificato: "rossi" → atleta, "judo" → gruppo + gara.
- 7.7 ✅ Registro attività. Migration `20260914210000_audit_rpc.sql`: `list_audit_logs(p_org, p_limit)` SECURITY DEFINER (risolve l'autore da `profiles` nonostante `profiles_self`; per gli UPDATE elenca i campi cambiati escludendo `updated_at`). `/impostazioni/audit` (permesso `organization.manage`) con tabella etichettata. Verificato: 48 voci, autore risolto, diff campi corretto (es. "sent_at, status").
- **Import controllato: rinviato alla Fase 8** (o post-lancio).
